...
public class TeachersController {
    @FXML private ListView<HBox> teacherListView;
    ...
    public void handleAddTeacher() {
        ...
        showAlert(Alert.AlertType.INFORMATION, "Feature Coming Soon", "Add Teacher functionality will be available soon.");
    }

    public void loadTeachers() {
        ObservableList<HBox> teacherCards = FXCollections.observableArrayList();

        try (Connection conn = DatabaseConnection.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT нэр, овог, имэйл, төрсөнОнСар, Хаяг, Зураг FROM Багш WHERE статус='teacher'")) {

            while (rs.next()) {
                HBox teacherCard = createPersonCard(
                        rs.getString("нэр"),
                        rs.getString("овог"),
                        rs.getString("имэйл"),
            ...
            Platform.runLater(() -> teacherListView.setItems(teacherCards));

        } catch (Exception e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Error", "Failed to load teacher data.");
        }
    }
